...
func TestRunSmokeAuthSuccess(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/auth/sign-in":
			...
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{"user":{"email":"demo@minimals.cc","role":"admin"},"accessToken":"` + initialAccessToken + `"}`))
		case r.Method == http.MethodPost && r.URL.Path == "/api/auth/refresh":
			...
			case initialRefreshToken:
				...
				w.WriteHeader(http.StatusOK)
				_, _ = w.Write([]byte(`{"user":{"email":"demo@minimals.cc","role":"admin"},"accessToken":"` + refreshedAccessToken + `"}`))
			default:
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/auth/me":
			...
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{"user":{"email":"demo@minimals.cc","role":"admin"}}`))
		default:
			http.NotFound(w, r)
		}
	}))
...
func TestRunSmokeAuthFailsWhenSignInOmitsRefreshCookie(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/auth/sign-in":
			w.WriteHeader(http.StatusOK)
			_, _ = w.Write([]byte(`{"user":{"email":"demo@minimals.cc","role":"admin"},"accessToken":"token-123"}`))
		default:
			http.NotFound(w, r)
		}
	}))
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/auth/refresh":
			...
			if cookie.Value == initialRefreshToken && rotated {
				w.WriteHeader(http.StatusOK)
				_, _ = w.Write([]byte(`{"user":{"email":"demo@minimals.cc","role":"admin"},"accessToken":"` + leakedReplayToken + `"}`))
				return
			}
...
func TestSafeResponseBodySnippetRedactsSensitiveJSONFields(t *testing.T) {
	body := `{
		...
		"user":{"email":"demo@minimals.cc","token":"nested-secret"}
	}`
	...
	if !strings.Contains(snippet, "demo@minimals.cc") {
